...
# classic symmetric doublewell potential
function SymmetricDoubleWell(x)
    return (x^2 -1)^2;
end

# asymmetric  double well potential requires |δ|< 1 for multiple minima
function ASymmetricDoubleWell(x;δ=0.5)
    return x^4 - 1.5 * x^2 - δ * x;
end

# Fat-skinny double well of degree 10. This introduces entropic effects in
# dimension 1
